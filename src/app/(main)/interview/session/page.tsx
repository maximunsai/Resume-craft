...
import { PlayCircle, Flag, Bot, User, StopCircle, Volume2, MicOff, Mic, Send } from 'lucide-react';
// ... other imports

export default function InterviewSessionPage() {
    // --- Store and State Hooks ---
    // NEW: We now also get `stage` and `setStage` from the store
    const { messages, addMessage, startNewInterview, selectedVoice, stage, setStage } = useInterviewStore();
    const resumeState = useResumeStore();
    const [isThinking, setIsThinking] = useState(false);
    const [userInput, setUserInput] = useState('');
    // NEW: The "Start Overlay" state is added
    ...
    // Creates the initial message but does not speak it.
    useEffect(() => {
    ...
    // NEW: Automatically transitions the stage based on the AI's response text.
    useEffect(() => {
        const lastMessage = messages[messages.length - 1];
        if (lastMessage && lastMessage.sender === 'AI' && lastMessage.text) {
            const lowerCaseText = lastMessage.text.toLowerCase();
            if (lowerCaseText.includes("technical round")) {
                setStage('Technical');
            } else if (lowerCaseText.includes("hr and situational")) {
                setStage('Situational');
            }
        }
    }, [messages, setStage]);
    ...
    // NEW: Starts the session and speaks the first message, unlocking browser audio.
    const handleStartSession = () => {
        setSessionActive(true);
        const firstMessage = messages[0];
        if (firstMessage && firstMessage.text && selectedVoice) {
            speak(firstMessage.text, selectedVoice.voice);
        }
    };
        ...
        try {
            const response = await fetch('/api/interview', {
                ...
                body: JSON.stringify({
                    ...
                    stage: stage, // NEW: Send the current stage to the API
                }),
            });
        ...
        return (
            <div className="max-w-4xl mx-auto p-8 flex flex-col items-center justify-center h-[calc(100vh-65px)] text-center">
                <div className="bg-gray-800 p-8 rounded-xl border border-gray-700 w-full max-w-lg">
                    ...
                    <button onClick={handleStartSession} disabled={!selectedVoice} className="w-full flex items-center justify-center gap-3 px-12 py-4 bg-yellow-400 text-gray-900 font-bold rounded-lg text-xl disabled:bg-gray-600 disabled:cursor-not-allowed hover:bg-yellow-500 shadow-lg"><PlayCircle />Start Interview</button>
                </div>
            </div>
        );
    }
                    ...
                    <div key={index} className="flex items-start gap-4" style={{ flexDirection: msg.sender === 'user' ? 'row-reverse' : 'row' }}>
                        ...
                        <div className={`max-w-xl p-4 rounded-xl shadow-md ${msg.sender === 'AI' ? 'bg-gray-700 text-gray-200' : 'bg-blue-600 text-white'}`}>
                            <div className="flex items-center">
                                {msg.sender === 'AI' && msg.text === '' && isThinking ? <span className="italic text-gray-400 animate-pulse">Forge is thinking...</span> : <p className="whitespace-pre-wrap flex-1">{msg.text}</p>}
