...
export default function InterviewSessionPage() {
    ...
    useEffect(() => {
        if (speechToText) {
            setUserInput(prev => (prev ? prev + ' ' : '').trim() + speechToText);
        }
    }, [speechToText]);

    // Value to display in the textarea, combining final and interim text
    const displayedInput = isListening ? (userInput.endsWith(' ') ? userInput : userInput + ' ') + interimText : userInput;

    // Effect to start the interview on first load
    useEffect(() => {
    ...
    const handleSubmitAnswer = async () => {
        ...
        // Ensure all voice I/O is stopped before proceeding
        ...
        setUserInput('');
        setIsThinking(true);
        addMessage({ sender: 'AI', text: '' }); // Add a placeholder for the streaming response
        ...
        try {
            ...
            while (true) {
                ...
                // Update the last AI message in the store with the streaming text
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 flex flex-col h-[calc(100vh-65px)]">
            <h1 className="text-2xl font-bold text-white text-center mb-4 flex-shrink-0">Mock Interview Session</h1>
                ...
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-4 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                        ...
                        {msg.sender === 'AI' && msg.text && ttsSupported && (
                           <button onClick={() => isSpeaking ? cancelSpeech() : speak(msg.text)} className={`p-2 rounded-full flex-shrink-0 self-center transition-colors ${isSpeaking ? 'bg-red-500/50 text-white' : 'bg-gray-600 text-gray-400 hover:text-white'}`}>
            ...
            <div className="flex-shrink-0 border border-t-0 border-gray-700 rounded-b-lg p-4 bg-gray-800">
                <div className="relative">
                    <textarea
                        ...
                        placeholder={isListening ? "Listening..." : (isThinking ? "Waiting for AI..." : "Type or speak your answer...")}
