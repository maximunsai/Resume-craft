...
export const useResumeStore = create<ResumeState>((set, get) => {
    ...
    const triggerSave = () => {
        ...
        debounceTimer = setTimeout(() => saveDataToDb(get()), 1500);
    };
    ...
    return {
        ...
        initialize: async () => {
            ...
            const { data: { session } } = await supabase.auth.getSession();
            if (!session) { set({ isInitialized: true }); return; }
        ...
        setPersonal: (details) => { set(state => ({ personal: { ...state.personal, ...details } })); triggerSave(); },
        addExperience: () => { set(state => ({ experience: [...state.experience, { ...initialState.experience[0], id: Date.now() }] })); triggerSave(); },
        updateExperience: (id, field, value) => { set(state => ({ experience: state.experience.map(exp => exp.id === id ? { ...exp, [field]: value } : exp) })); triggerSave(); },
        removeExperience: (id) => { set(state => ({ experience: state.experience.filter(exp => exp.id !== id) })); triggerSave(); },
        setSkills: (skills) => { set({ skills }); triggerSave(); },
        setFinalThoughts: (thoughts) => { set({ finalThoughts: thoughts }); triggerSave(); },
        setJobDescription: (jd) => { set({ jobDescription: jd }); triggerSave(); },
        populateFromParsedResume: (parsedData) => {
            ...
            if (parsedData.experience?.length > 0) newState.experience = parsedData.experience.map((exp: any, i: number) => ({ id: Date.now() + i, company: exp.company || '', title: exp.title || '', startDate: exp.startDate || '', endDate: exp.endDate || '', description: exp.description || '' }));
